...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream()).map(Item::getName).filter(itemName -> itemListOnSale.stream().noneMatch(itemName::equals)).collect(Collectors.toSet());
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() >= customer.getWantToBuy().stream()
                .mapToInt(itemWant -> onSale.stream().filter(shopItem -> shopItem.getName().equals(itemWant.getName())).min(Comparator.comparingInt(Item::getPrice)).map(Item::getPrice).orElse(0)).sum();
